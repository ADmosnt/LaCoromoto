  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          { label: '+ Nueva Orden', color: 'bg-brand-600 hover:bg-brand-700', action: () => setOrdenModalOpen(true) },
      ...
      {data?.tasa_hoy && (
        <div className="bg-brand-50 border border-brand-200 rounded-lg p-4 mb-6 flex items-center justify-between flex-wrap gap-3">
          <div>
            <p className="text-sm text-brand-600 font-medium">Tasa BCV hoy ({data.tasa_hoy.fecha})</p>
            <p className="text-2xl font-bold text-brand-800">Bs. {Number(data.tasa_hoy.valor).toFixed(4)}</p>
          </div>
          <span className="text-xs bg-brand-100 text-brand-600 px-2 py-1 rounded">{data.tasa_hoy.fuente}</span>
        </div>
      )}
                ...
                {data.ultimos_reportes.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3 text-center">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${r.status === 'confirmado' ? 'bg-brand-100 text-brand-700' : 'bg-yellow-100 text-yellow-700'}`}>
